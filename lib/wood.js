...
module.exports = (function setup() {
  ...
  EX.describeDetails = function (wood) {
    ...
    return (padstr('#', (wood.parent && Number.isFinite(wood.childIndex)
        && String(wood.childIndex)), ' ') +
      padstr('<', wood.tagName, '> ') +
  ...
  EX.reindexChildren = function (opt, par, offset) {
    var idx, ch, onto = opt.onto;
    if (opt.length !== undefined) { throw new Error('bad opt: length'); }
    if (!is.fin(par.length)) { throw new Error('Parent needs a length'); }
  ...
  EX.abandonChildren = function abandon(parent, children) {
    ...
    parent.forEach(function backupToBeKeptChildren(ch, idx) {
      if (byIndex[idx]) { EX.forgetParent(ch); } else { children.push(ch); }
    });
    EX.reindexChildren({ onto: parent }, children, reindexFrom);
  ...
  EX.abandonChildren.justOne = function (parent, child) {
    ...
    EX.reindexChildren(false, parent, child.idx);
    return parent;
  };
