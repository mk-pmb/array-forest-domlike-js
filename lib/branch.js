...
module.exports = (function setup() {
  ...
  api.at = api.getAttribute = function getAttribute(attrName, ifUndef) {
    if (attrName === undefined) { return (this.N.attrib || false); }
    var val = (this.N.attrib || false)[attrName];
  ...
  api.nc = api.countChildren = function () {
  ...
  api.filter = function (opt) {
    return woodFilter(this.N, opt, { convertList: ArfNodeList,
  ...
  api.childById = function (id) {
    return this.filter({ attrib: { id: id }, nthResult: 1 });
  };
  api.id = function (id) {
    return (id === undefined ? this.at('id', '') : this.childById(id));
  };


  api.childrenByName = function (name, idx) {
    return this.filter({ attrib: { name: name },
      nthResult: (Number.isFinite(idx) && (idx + 1)) });
  };
  api.nm = function (name, idx) {
    return (name === undefined ? this.at('name', '')
      : this.childrenByName(name, idx));
  };
  ...
  api.sib = function (distance) {
    return (distance === undefined ? this.allSiblings()
      : this.siblingByDistance(distance));
  };
